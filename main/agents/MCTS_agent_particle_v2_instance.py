...
class MCTS_agent_particle_v2_instance:
    ...
    def update_human_mind(self, base_obs, humanID, human_obs, last_action_dict, subgoal_space, resample=True):
        '''
        TODO: Need to represent particles as (mind, P(mind))
            This requires extracting P(state belief) * P(g_{agent}) * P(g_{human})
                Currently able to get P(state belief)

        Also need to add belief resampling when seeing a new agent in gt that you didn't see before
        '''

        if None in self.human_mind_particles or resample:  # we have an empty set of particles, need to sample
            for s in range(self.num_particles):
                # TODO: Obs needs to be the perspective of the human from the ground truth initial graph
                humanBelief = state_belief.make_partial_belief(base_obs, humanID, human_obs, self.belief_params)

                belief_prob = 1.0  # TODO: extract belief prob from edge weights
                '''
                TODO:
                    agent_belief should be sampled from distribution of human inferring robot
                    human_goal should be sampled from distribution of robot inferring human
                '''
                humanMind = Mind.Mind(
                    agent_belief=humanBelief,
                    agent_goal=random.choice(subgoal_space) if subgoal_space is not None else None,
                    human_goal=random.choice(subgoal_space) if subgoal_space is not None else None)

                self.human_mind_particles[s] = (humanMind, belief_prob)
        if last_action_dict is not None:
            for (humanMind, belief_prob) in self.human_mind_particles:
                # extract the sampled state
                # assumes last action is a dictionary mapping agentID --> action string
                # TODO: must be action dict from human's perspective
                state_belief.simulate_partial_belief(humanMind.agent_belief, last_action_dict, self.sim_env)
    ...
    def get_action(
        ...
        # TODO: substitute parameters with correct values described in function
        self.update_human_mind(
            obs=obs, 
            humanID=0 if (self.agent_id == 1) else 1, 
            human_obs=other_obs,
            last_action_dict=last_action, 
            subgoal_space=subgoals,
            resample=True)

        # print([b[1] for b in self.human_mind_particles])
        #pdb.set_trace()

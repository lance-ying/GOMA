...
def get_rooms(id2node, belief_type, room_ids):
    if belief_type == "uniform":
        ...
        id_kitchen = [
            (id_room, index_cont)
            for index_cont, id_room in enumerate(room_ids)
            if id_room != None and id2node[id_room]["class_name"] == "kitchen"
        ]
        if len(id_kitchen) > 0:
...
def get_container_prior(id2node, belief_type, container_ids):
    if belief_type == "uniform":
        ...
        try:
            id_cabinet = [
                (id_obj, index_cont)
                for index_cont, id_obj in enumerate(container_ids)
                if id_obj != None and id2node[id_obj]["class_name"] == "cabinet"
            ]
        except:
            ipdb.set_trace()
        if len(id_cabinet) > 0:
        ...
        try:
            id_cabinet = [
                (id_obj, index_cont)
                for index_cont, id_obj in enumerate(container_ids)
                if id_obj != None and id2node[id_obj]["class_name"] in class_names
            ]
        except:
            ipdb.set_trace()
        if len(id_cabinet) > 0:
...
class Belief:
    def __init__(self, graph_gt, agent_id, prior=None, seed=None, belief_params={}):
        ...
        self.container_restrictions = {"book": ["cabinet", "kitchencabinet"]}
        ...
        self.house_obj = ["floor", "wall", "ceiling"]
        ...
        self.prohibit_ids = [
            ...
            if node["class_name"].lower() in self.class_nodes_delete
            or node["category"] in self.categories_delete
        ]
        new_graph = {
            "nodes": [
                copy.deepcopy(node)
                for node in graph_gt["nodes"]
                if node["id"] not in self.prohibit_ids
            ],
            "edges": [
                edge
                for edge in graph_gt["edges"]
                if edge["to_id"] not in self.prohibit_ids
                and edge["from_id"] not in self.prohibit_ids
            ],
        }
        ...
        for bin_var in self.binary_variables:
            self.bin_var_dict[bin_var.negative] = [
                [bin_var.positive, bin_var.negative],
                1,
            ]
            self.bin_var_dict[bin_var.positive] = [
                [bin_var.positive, bin_var.negative],
                0,
            ]
    ...
    def update(self, origin, final):
        ...
        signdelta = (final_sm - origin_sm) * 1.0 / (maxdelta + 1e-9)
    ...
    def update_to_prior(self):
        for node_name in self.edge_belief:
            for keys in self.edge_belief[node_name]:
                self.edge_belief[node_name][keys][1] = self.update(
                    self.edge_belief[node_name][keys][1],
                    self.first_belief[node_name][keys][1],
                )
            # self.edge_belief[node_name]["INSIDE"][1] = self.update(
            #     self.edge_belief[node_name]["INSIDE"][1],
            #     self.first_belief[node_name]["INSIDE"][1],
            # )

            # self.edge_belief[node_name]["ON"][1] = self.update(
            #     self.edge_belief[node_name]["ON"][1],
            #     self.first_belief[node_name]["ON"][1],
            # )

        for node in self.room_node:
            self.room_node[node][1] = self.update(
                self.room_node[node][1], self.first_room[node][1]
            )
    ...
    def _remove_house_obj(self, state):
        delete_ids = [
            ...
            if x["class_name"].lower() in self.class_nodes_delete
        ]
    ...
    def init_belief(self):
        ...
        for node in self.sampled_graph["nodes"]:
            ...
            for bin_var in bin_vars:
                if bin_var.positive == "OFF" and "light" not in object_name:
                    ...
                    belief_dict[bin_var.positive] = 1.0  # Lights on
                else:
                    belief_dict[bin_var.positive] = 0.0  # Objectd are closed
        ...
        object_containers = [
            node
            for node in self.sampled_graph["nodes"]
            if node["class_name"] in container_classes
        ]
        object_surfaces = [
            node
            for node in self.sampled_graph["nodes"]
            if node["class_name"] in surface_classes
        ]
        ...
        self.room_nodes = [
            node for node in self.sampled_graph["nodes"] if node["category"] == "Rooms"
        ]
        ...
        self.container_index_belief_dict = {
            x: it for it, x in enumerate(self.container_ids) if x is not None
        }
        self.surface_index_belief_dict = {
            x: it for it, x in enumerate(self.surface_ids) if x is not None
        }
        ...
        for obj_name in self.container_restrictions:
            ...
            restricted_ids = [
                x["id"]
                for x in object_containers
                if x["class_name"] not in possible_classes
            ]
        ...
        nodes_inside_ids = [
            x["from_id"]
            for x in self.sampled_graph["edges"]
            if x["to_id"] not in self.room_ids and x["relation_type"] == "INSIDE"
        ]
        nodes_inside = [
            node
            for node in self.sampled_graph["nodes"]
            if node["id"] in nodes_inside_ids and "GRABBABLE" not in node["states"]
        ]
        ...
        for node in objects_for_belief_reasoning:
            if (
                node["class_name"] in self.class_nodes_delete
                or node["category"] in self.categories_delete
            ):
                continue
            id1 = node["id"]
            ...
            try:
                init_values = get_container_prior(
                    id2node, self.belief_type, self.container_ids
                )
            except:
                pdb.set_trace()
            ...
            if node["class_name"] in self.id_restrictions_inside.keys():
                try:
                    init_values[
                        self.id_restrictions_inside[node["class_name"]]
                    ] = self.low_prob
                except:
                    continue
        ...
        for node in self.sampled_graph["nodes"]:
            ...
            if node not in self.room_nodes:
                if node["id"] in self.container_ids and self.knowledge_containers:
                    ...
                    room_index = [
                        it
                        for it, index in enumerate(self.room_ids)
                        if index == room_container[0]
                    ][0]
    ...
    def sample_from_belief(self, as_vh_state=False, ids_update=None, obs=None, return_prob=False):
        ...
        sample_prob = 1.0
        get_choice_prob = lambda x,y,z: z[y.index(x)]
        ...
        for node in self.sampled_graph["nodes"]:
            ...
            for var_name, var_belief_value in belief_node.items():
                ...
                #pdb.set_trace()
                if (rand_number < var_belief_value):
                    sample_prob *= var_belief_value
                    value_binary = 1
                else:
                    value_binary = 0
                    sample_prob *= (1 - var_belief_value)
        ...
        for node in self.sampled_graph["nodes"]:
            ...
            if node["id"] not in self.edge_belief:
                ...
                node_room = np.random.choice(
                    node_room_cands[0], p=scipy.special.softmax(node_room_cands[1])
                )
                #pdb.set_trace()
                sample_prob *= get_choice_prob(node_room, node_room_cands[0], scipy.special.softmax(node_room_cands[1]))
                ...
                if node["id"] in node_inside:
                    ...
                    try:
                        sample_inside = np.random.choice(
                            edge_belief_inside[0],
                            p=scipy.special.softmax(edge_belief_inside[1]),
                        )
                        #pdb.set_trace()
                        sample_prob *= get_choice_prob(sample_inside, edge_belief_inside[0], scipy.special.softmax(edge_belief_inside[1]))
                    except:
        ...
        for edge in self.sampled_graph["edges"]:
            if (
                ...
                and edge["from_id"] in self.surface_ids
                and edge["to_id"] in self.room_ids
            ):
        ...
        for node_id, room_id in in_room:
            ...
            node_surface = np.random.choice(
                surface_cands[0], p=scipy.special.softmax(surface_cands[1])
            )
            #pdb.set_trace()
            sample_prob *= get_choice_prob(node_surface, surface_cands[0], scipy.special.softmax(surface_cands[1]))
        ...
        for node_door in self.door_edges.keys():
            ...
            self.sampled_graph["edges"].append(
                {"to_id": node_1, "from_id": node_door, "relation_type": "BETWEEN"}
            )
            self.sampled_graph["edges"].append(
                {"to_id": node_2, "from_id": node_door, "relation_type": "BETWEEN"}
            )

        if as_vh_state and return_prob:
            return self.to_vh_state(self.sampled_graph), sample_prob
        elif as_vh_state and not return_prob:
            return self.to_vh_state(self.sampled_graph)
        elif not as_vh_state and return_prob:
            return self.sampled_graph, sample_prob

        return self.sampled_graph

    def to_vh_state(self, graph):
        state = self._remove_house_obj(graph)
        vh_state = EnvironmentState(
            EnvironmentGraph(state), self.name_equivalence, instance_selection=True
        )
        return vh_state
    ...
    def update_belief(self, gt_graph, update_mind=False):
        self.update_to_prior()
        self.update_from_gt_graph(gt_graph, update_mind=update_mind)

    def update_graph_from_gt_graph(
        self,
        gt_graph,
        sampled_graph=None,
        ...
        gt_graph = {
            ...
            "edges": [
                ...
                if edge["from_id"] not in self.prohibit_ids
                and edge["to_id"] not in self.prohibit_ids
            ],
        }
        ...
        for x in edges_gt_graph:
            if x["relation_type"] == "INSIDE":
                if x["from_id"] in inside.keys() and id2node[inside[x["from_id"]]]['category'] != 'Rooms':
                    try:
                        if inside[inside[x["from_id"]]] == x['to_id']:
                            continue
                    except:
                        inside[inside[x["from_id"]]] = x["to_id"]
                        continue
        ...
        if resample_unseen_nodes:
            ids_to_update = list(
                set([node["id"] for node in self.sampled_graph["nodes"]])
                - set(ids_visible)
            )
        else:
            for edge in self.sampled_graph["edges"]:
                ...
                if (
                    edge["from_id"] == char_node or edge["to_id"] == char_node
                ...
                if edge["to_id"] in id2node.keys() and edge["to_id"] != char_node:
                    ...
                    if (
                        id2node[edge["to_id"]]["category"] == "Rooms"
                        and edge["relation_type"] == "INSIDE"
                    ):
                        ...
                        if (
                            edge["relation_type"] == "INSIDE"
                            and "OPEN" in id2node[edge["to_id"]]["states"]
                        ):
                            ids_to_update.append(edge["from_id"])
                            continue
        ...
        nodes_close = [
            x["to_id"]
            for x in edges_gt_graph
            if x["from_id"] == char_node and x["relation_type"] == "CLOSE"
        ]
        inside_belief = {}
        for edge in edges_keep:
            if edge["relation_type"] == "INSIDE":
                if edge["to_id"] not in inside_belief:
                    inside_belief[edge["to_id"]] = []
        ...
        output = self.sample_from_belief(ids_update=ids_to_update, return_prob=return_prob)
    ...
    def update_from_gt_graph(self, gt_graph, update_mind=False):
        ...
        for x in gt_graph["edges"]:
            ...
            if x["relation_type"] == "INSIDE":
                if x["from_id"] in inside.keys() and id2node[inside[x["from_id"]]]['category'] != 'Rooms':
                    # item is already in something in a room
                    try:
                        if inside[inside[x["from_id"]]] == x['to_id']:
                            continue
                    except:
            ...
            if x["relation_type"] == "ON" and x["to_id"] in self.surface_ids:
                if (
                    x["from_id"] in on.keys()
                    and x["from_id"] in self.edge_belief.keys()
                    and x["to_id"] in self.surface_ids
                ):
                    if on[on[x["from_id"]]] == x['to_id']:
                        continue
                    print(
                        "Already on",
                        id2node[x["from_id"]]["class_name"],
        ...
        for id_node in self.edge_belief.keys():
            ...
            if id_node in visible_ids:
                ...
                #print("hellobelief:",id_node)
                #if id_node not in inside.keys():
                    #print(id_node)
                ...
                # inside the room
                if (
                    inside_obj not in self.room_ids
                    and inside_obj not in self.container_index_belief_dict
                ):
                ...
                if inside_obj in self.room_ids:
                    if "INSIDE" not in self.edge_belief[id_node]:
                        self.edge_belief[id_node]["INSIDE"] = [[None], np.array([1.0])]
                    else:
                        self.edge_belief[id_node]["INSIDE"][1][:] = self.low_prob
                        self.edge_belief[id_node]["INSIDE"][1][0] = 1.0
                    self.room_node[id_node][1][:] = self.low_prob
                    self.room_node[id_node][1][
                        self.room_index_belief_dict[inside_obj]
                    ] = 1.0
                else:
                    ...
                    self.edge_belief[id_node]["INSIDE"][1][:] = self.low_prob
                    self.edge_belief[id_node]["INSIDE"][1][index_inside] = 1.0

                if id_node in on.keys():
                    # object is on something
                    on_obj = on[id_node]
                    index_on = self.surface_index_belief_dict[on_obj]
                    if "ON" not in self.edge_belief[id_node]:
                        # simulating what it's on if that edge hasn't been added
                        temp_on_items = [None] * (index_on + 1)
                        temp_on_items[index_on] = on_obj
                        temp_probs = [1/(index_on + 1)] * (index_on + 1)
                        self.edge_belief[id_node]["ON"] = [temp_on_items, np.array(temp_probs)]

                    self.edge_belief[id_node]["ON"][1][:] = self.low_prob
                    self.edge_belief[id_node]["ON"][1][index_on] = 1.0
                else:
                    # object is for suure on nothing
                    if 'ON' not in self.edge_belief[id_node]:
                        self.edge_belief[id_node]["ON"] = [[None], np.array([1.0])]
                    else:
                        self.edge_belief[id_node]["ON"][1][:] = self.low_prob
                        self.edge_belief[id_node]["ON"][1][0] = 1.0
                ...
                self.room_node[id_node][1][
                    self.room_index_belief_dict[visible_room]
                ] = self.low_prob
        ...
        for id_node in self.container_ids:
            if id_node in visible_ids and "OPEN" in id2node[id_node]["states"]:
                for id_node_child in self.edge_belief.keys():
                    if (
                        id_node_child not in inside.keys()
                        or inside[id_node_child] != id_node
                    ):
                        ids_known_info[1].append(
                            self.container_index_belief_dict[id_node]
                        )
            ...
            if id_node in visible_ids:
                for id_node_child in self.edge_belief.keys():
                    if id_node_child not in on.keys() or on[id_node_child] != id_node:
                        ids_known_info[1].append(
                            self.surface_index_belief_dict[id_node]
                        )
                        try:
                            self.edge_belief[id_node_child]["ON"][1][
                                self.surface_index_belief_dict[id_node]
                            ] = self.low_prob
                        except:
                            pass
        ...
        #     except:
        #         for temp_id in range(len(mask_obj)):
        #             try:
        #             mask_obj[ids_known_info[1][temp_id]] = 0
        ...
        for id_node in self.room_node.keys():
            if id_node in self.edge_belief.keys():
                ...
                if np.max(self.edge_belief[id_node]["INSIDE"][1]) == self.low_prob:
                    ...
                    self.edge_belief[id_node]["INSIDE"][1] = self.first_belief[id_node][
                        "INSIDE"
                    ][1]
                    # Sample rooms except marked
                    try:
                        self.room_node[id_node][1][mask_house] = self.first_room[
                            id_node
                        ][1][mask_house]
                    except:
                        pdb.set_trace()
                ...
                if np.max(self.room_node[id_node][1]) == self.low_prob:
                    self.room_node[id_node][1][mask_house] = self.first_room[id_node][
                        1
                    ][mask_house]

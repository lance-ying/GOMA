...
import sys
sys.path.append("..")
#import gpt_message_encoder
#sys.path.append("..")
from GPT_message import gpt_message_encoder
import json
import eventlet
...
save_messages={"template_message":[],"natural_message":[]}
global saving_step
saving_step=50
global GPT_resting_step
GPT_resting_step=100
...
class ArenaMP(object):
    def __init__(
        ...
        self.agent_communicate = agent_communicate
        ...
        self.init_graph = self.env.get_graph()
    ...
    @property
    def all_goal_objects_name(self):
        goal_objects = []
        id2node = {node['id']:node for node in self.env.full_graph['nodes']}
        for predicate in self.env.goal_spec[0]:
    ...
    def reset(self, task_id=None):
        ...
        for it, agent in enumerate(self.agents):
            if agent.agent_type != "COELA":
                ...
                goals_to_process = self.env.goal_spec[it]
                if len(goals_to_process) == 0:
                    goals_to_process = self.env.task_goal[0]

                print(self.all_goal_objects_name)
                print(goals_to_process)
                # ipdb.set_trace()
                agent.reset(
                    ob[it], self.env.full_graph, self.all_goal_objects_name, goals_to_process
                )
        return ob
    ...
    def get_actions(
        ...
        # print(1/0)
        # ipdb.set_trace()
        dict_actions, dict_messages, dict_info = {}, {}, {}
        ...
        for it, agent in enumerate(self.agents):
            ...
            full_id2node = {x['id']:x for x in full_graph['nodes']}

            partial_graph = obs[it]
            partial_id2node = {x['id']:x for x in partial_graph['nodes']}

            acceptable_categories = {'Walls', 'Furniture', 'Appliances', 'Ceiling', 'Floor', 'Decor', 
            'Windows', 'Lamps', 'Rooms', 'Doors', 'Electronics'}

            edges_added = set((x['from_id'], x['to_id']) for x in obs[it]['edges'])

            current_room_id = None
            curr_location = full_id2node[it+1]['obj_transform']['position']

            # agents should see all acceptable nodes if they haven't already
            for node_id, node in full_id2node.items():
                if node['category'] in acceptable_categories and node_id not in partial_id2node:
                    obs[it]['nodes'].append(node)  # add acceptable nodes
                    partial_id2node[node_id] = node  # mark in dictionary

            for edge in full_graph['edges']:  # add all edges in acceptable categories
                from_id = edge['from_id']
                to_id = edge['to_id']
                if from_id == (it+1) and full_id2node[to_id]['category'] == 'Rooms':
                    current_room_id = to_id
                if to_id == (it+1) and full_id2node[from_id]['category'] == 'Rooms':
                    current_room_id = from_id

                if (from_id, to_id) in edges_added:  # don't waste computation
                    continue
                elif full_id2node[from_id]['category'] not in acceptable_categories:
                    continue
                elif full_id2node[to_id]['category'] not in acceptable_categories:
                    continue
                else:
                    obs[it]['edges'].append(edge)
                    edges_added.add((from_id, to_id))


            # character_to_room = {}
            # for cId, node in partial_id2node.items():  # trying to find which room agents are in
            #     if node['category'] == 'Characters':
            #         for edge in obs[it]['edges']:
            #             toEval = None
            #             if edge['from_id'] == cId:
            #                 toEval = edge['to_id']
            #             elif edge['to_id'] == cId:
            ...
            #             else:
            #                 continue
            #             if partial_id2node[toEval]['category'] == 'Rooms':
            #                 character_to_room[cId] = toEval
            #                 break

            # def give_full_obs():
            #     for rID in character_to_room.values():
            #         for edge in full_graph['edges']:
            #             from_id = edge['from_id']
            #             to_id = edge['to_id']
            #             if from_id == rID or to_id == rID:  # only care about edges in same room as agent
            #                 if edge not in obs[it]['edges']:
            #                     obs[it]['edges'].append(edge)
            #                     if from_id not in partial_id2node:
            #                         obs[it]['nodes'].append(full_id2node[from_id])
            #                     if to_id not in partial_id2node:
            #                         obs[it]['nodes'].append(full_id2node[to_id])

            # if len(character_to_room) == 1:
            #     give_full_obs()
            ...
            if agent.agent_type == "MCTS_human_message":
                satisfied, unsatisfied = utils_env.check_progress2(
                    full_graph, goal_spec
                )
                all_satisfied = all([x <= 0 for x in unsatisfied.values()])
                # if all_satisfied:
                #     pdb.set_trace()
                (
                    dict_actions[it],
                ...
                messages = {}
               # message_type_list = ["request", "mistake", "ask_info", "check_progress", "share_info"]
                # message_type_list = ["request", "ask_info", "share_info"]
                if len(prev_messages) == 0:
                    dict_messages[it] = agent.gen_human_message(
                            "request_help",
                            obs_prev_actions,
                            prev_messages,
                            goal_spec,
                            agent.goal_predicates_helper,
                        )
                else:

                    if self.agent_communicate:
                        dict_messages[it] = agent.gen_human_message(
                                "share_info",
                                obs_prev_actions,
                                prev_messages,
            ...
            elif agent.agent_type == 'COELA':
                humanID = 0 if it == 1 else 1
                satisfied, unsatisfied = utils_env.check_progress2(
                    full_graph, goal_spec
                )
                all_satisfied = all([x <= 0 for x in unsatisfied.values()])
                ...
                if len(agent.dialogue_history) == 0:
                    dict_actions[it] = None
                    dict_messages[it] = None
                else:
                    dict_messages[it] = message


            elif agent.agent_type == "MCTS_helper_message":
                if self.single_agent:
                    dict_actions[it], dict_info[it] = None, None
                else:
                    start_time = time.time()
                    humanID = 0 if it == 1 else 1

                    try:
                        prev_human_actions = obs_prev_actions[it][humanID]
                    except:
                        prev_human_actions = None
                    ...
                    (
                        ...
                        length_plan=length_plan,
                        must_replan=True,
                        prev_actions=prev_human_actions, # store observed human actions for inference
                        return_inferred_goal=True
                        # must_replan=False if must_replan is None else must_replan[it],
                    )

                    if self.agent_communicate:

                        dict_messages[it] = agent.gen_helper_message(
                            obs_prev_actions,
                            prev_messages,
                            obs[humanID+1],
                            obs[humanID],
                ...
                if "MCTS" in agent.agent_type or "Random" in agent.agent_type:
                    if true_graph:
                        ...
                        full_graph = None
                    dict_actions[it], _, dict_info[it] = agent.get_action(
                        obs[it],
                        goal_spec,
        ...
        # print("DDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDDdict_messages:",dict_messages)
        ...
        return dict_actions, dict_messages, dict_info
    ...
    def step(self, true_graph=False):
        ...
        obs, obs_prev_actions, prev_messages = self.env.get_observations()

        # print(obs)

        action_space = self.env.get_action_space()
        dict_actions, dict_messages, dict_info = self.get_actions(
            obs, obs_prev_actions, prev_messages, action_space, true_graph=true_graph
        )

        try:
            step_info = self.env.step(dict_actions, dict_messages)
            # for agent_id, agent in enumerate(self.agents):
            #    agent.step(dict_actions[agent_id])

        except:
            raise utils_exception.UnityException
        return step_info, dict_actions, dict_messages, dict_info
    ...
    def run(self, random_goal=False, pred_goal=None, save_img=None):
        ...
        while True:
            ...
            step += 1
            (
                (obs, prev_messages, reward, done, infos),
                actions,
                messages,
                agent_info,
            ) = self.step()

            print("Action, Message", actions,messages)
            # ipdb.set_trace()
            ...
            if actions[0] == prev_action and actions[0]!=None:
                num_repeated += 1
                if distance < 0.3:
                    num_nomove += 1
            else:
            ...
            for agent_id in range(self.num_agents):
              for key in messages[agent_id].keys():
                  if messages[agent_id][key]:
                      with open("../GPT_message/prompt.txt", "r", encoding='utf-8') as f:
                          prompt=f.read()
                          
                      prompt=prompt+messages[agent_id][key]
                      
                      #print("Instuction= \'"+key+"\': "+"\'"+messages[agent_id][key]+"\'\nTranslation= ")

                      #print(prompt)
                      new_message[agent_id][key]=gpt_message_encoder.generate_chat_response(prompt)

                      #print("=====================",gpt_message_encoder.generate_chat_response(prompt)+"\n"+"\n","=======================")
                      
                  else:
                      pass
            '''
            #print("GPT_message:", new_message)
            ...
            print("Plan (Main):", agent_info[0]["plan"][:4])
            print("----------")
            ...
            for agent_id, message in messages.items():
                saved_info["message"][agent_id].append(message)
                ...
                #    saved_info['belief_graph'][agent_id].append(info['belief_graph'])
                    ...
                    if "plan" in info:
                        if info['plan'] is None:
                            saved_info["plan"][agent_id].append(None)
                        else:
                    ...
                    if "obs" in info:
                        ...
                        try:
                            ...
                            saved_info['obs'].append(info['obs']['see_this_step'])
